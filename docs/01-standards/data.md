...
2. **`constants/` is the source of truth.** It owns the site's career data outright. The career-notes package (`ops/notes/tkodev/career-notes/profiles/linkedin`) seeded the initial types and constants and stays a reference for facts not yet captured here.
