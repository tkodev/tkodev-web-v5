...
2. **State discipline.** zustand only for genuinely global state (the architecture owns which stores exist); everything ephemeral stays in local `useState`/URL params. No speculative stores.
